...
export function LoginSignUp(props) {
  ...
  async function handleSubmit(evt) {
    ...
    // JR:
    // needed to know if you were on login or signup conditionally
    // useNavigation was the wrong import, broke page
    // had very confusing formatting
      ...
      <Paper elevation={3} style={{ padding: 'var(--padding)' }}>
        ...
        <form>
          ...
          <TextField
            label="password"
            type="password"
            error={response.includes('password')}
            helperText={response.includes('password') ? response : null}
            margin="normal"
            required
            fullWidth
            variant="outlined"
          ...
          {form === 'signup' && (
            <>
              ...
              <TextField
                ...
                type="email"
                variant="outlined"
                error={response.includes('email')}
                helperText={response.includes('email') ? response : null}
                fullWidth
                margin="normal"
                value={email}
                onChange={(evt) => {
